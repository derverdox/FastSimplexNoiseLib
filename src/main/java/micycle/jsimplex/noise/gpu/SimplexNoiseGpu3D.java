...
public class SimplexNoiseGpu3D {
	...
	public static synchronized float[] calculate(float x, float y, float z, int width, int height, int depth,
			float frequency) {
	...
	public static synchronized float[] calculateOctaved(float x, float y, float z, int width, int height, int depth,
		...
		simplexKernelOctaved.setParameters(x, y, z, width, height, depth, frequency, weight);
		simplexKernelOctaved.execute(width * height * depth);
		return simplexKernelOctaved.getResult();
	}

	public static synchronized float[] calculateFastOctaved(float x, float y, float z, int width, int height, int depth,
			float frequency, float[] weight) {
		fastSimplexKernelOctaved.setParameters(x, y, z, width, height, depth, frequency, weight);
